...
- **`find()`**: Zwraca pozycję pierwszego wystąpienia podanego ciągu znaków gdzie wielkość liter ma znaczenie lub wartość `std::string::npos` jeżeli podana fraza nie zostanie znaleziona.
    ```cpp
   std::string str = "Hello, World!";
    int pos = str.find("World"); // pos = 7, bo "World" zaczyna się od 7 znaku w str 
    ```

- **`rfind()`**: Działa adekwatnie jak `find()` tylko zwraca ostatnie wstąpienie frazy w ciągu znaków - string.
    ```cpp
    std::string str = "Hello, World! World!";
    int pos = str.rfind("World"); // pos = 14
    ```

- **`substr(pos, len)`** zwraca ciąg znaków zaczynający się od pozycji `pos` i o długości `len`, jeżeli została podana..
    ```cpp
    std::string str = "Hello, World!";
